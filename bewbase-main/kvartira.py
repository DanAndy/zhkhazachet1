...
class Kvartira(QWidget):
    ...
    def insert(self):
        row = [self.leIDkv.text(), self.leFIO.text(), self.leAdress.text(), self.leTele.text(), self.leDate.text(), self.lePay.text()]
        try:
            cur = self.conn.cursor()
            cur.execute(f"""insert into Квартиросъемщики (IDkv, FIO, Adress, Telephone, DateOrder, PayMount)
